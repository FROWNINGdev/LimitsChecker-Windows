...
`RETRY_SECONDS` (для 401 — вчетверо реже). 429 и 5xx считаются временными и
повторяются внутри опроса с учётом `Retry-After`.
...
| `..._REFRESH_SECONDS` | `60` | Интервал опроса, сек (минимум 5) |
